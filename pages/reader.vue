...
const triggerSearch = async () => {
  const data: any = (await $fetch(`/api/hello?search=${lastValue.value}`)).items[0]
      console.log('request for ' + lastValue.value)
  if (!codes.value.map(code => code.code).includes(lastValue.value)) {
    codes.value.push({
      code: lastValue.value,
      resultOpenFF: data.title,
      img: data.pagemap?.imageobject && data.pagemap?.imageobject[0] && data.pagemap?.imageobject[0].thumbnail ? data.pagemap.imageobject[0].thumbnail : data.pagemap.metatags[0]['og:image']
    })
  }
}
  ...
  <div id="container">
    ...
    <ul>
      <li v-for="code in codes">
        {{ code.code }} {{ code.resultOpenFF }}
        <img :src="code.img" alt="img" />
      </li>
    </ul>

    <button @click="triggerSearch">Trigger search ! </button>
    <p>Search for {{ lastValue }} ?</p>
  </div>
</template>
